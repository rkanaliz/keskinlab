...
const IMAGE_TYPES=new Set(['sunum','ders-notu','infografik','hafta-ozeti']);
...
async function collectLeaf(course,week,rel,label){const dir=path.join(MATERIAL_ROOT,course,`hafta${week}`,...rel.split('/')),names=await filesIn(dir);if(!names.length)return[];if(IMAGE_TYPES.has(rel)&& (rel!=='ders-notu'||names.some(name=>RASTER_EXT.has(path.extname(name).toLowerCase())))){const imageNames=names.filter(name=>RASTER_EXT.has(path.extname(name).toLowerCase()));return imageNames.map(name=>{const ext=path.extname(name).toLowerCase(),index=path.basename(name,ext),href=publicPath('materyaller',course,`hafta${week}`,rel,name),derived=RASTER_EXT.has(ext)?webPaths(course,week,rel,name):null,downloads=[{href,format:ext.slice(1),filename:downloadName(course,week,label,ext,index)}];if(rel==='sunum'){const pdfName=`${index}.pdf`;if(names.includes(pdfName))downloads.push({href:publicPath('materyaller',course,`hafta${week}`,rel,pdfName),format:'pdf',filename:downloadName(course,week,label,'.pdf',index)})}return{type:rel,label,index,preview:DISPLAYABLE.has(ext)?href:null,webPreview:derived?.web??null,thumbnail:derived?.thumb??null,editable:EDITABLE.has(ext)?href:null,downloads}})}let metadata={};const manifest=path.join(dir,'_manifest.json');if(await exists(manifest)){try{metadata=JSON.parse(await fs.readFile(manifest,'utf8'))}catch(e){throw new Error(`${publicPath('materyaller',course,`hafta${week}`,rel,'_manifest.json')}: ${e.message}`)}}const manifestKeys=Object.keys(metadata);const resourceNames=manifestKeys.length?names.filter(name=>name!=='_manifest.json'&&manifestKeys.includes(path.basename(name,path.extname(name)))):names.filter(name=>name!=='_manifest.json');return groupFormats(resourceNames).map(([index,formats])=>{const itemMeta=metadata[index]||{},itemLabel=itemMeta.label||label,pdf=formats.pdf?publicPath('materyaller',course,`hafta${week}`,rel,formats.pdf):null,docx=formats.docx?publicPath('materyaller',course,`hafta${week}`,rel,formats.docx):null,downloads=[];if(pdf)downloads.push({href:pdf,format:'pdf',filename:downloadName(course,week,itemLabel,'.pdf',index)});if(docx)downloads.push({href:docx,format:'docx',filename:downloadName(course,week,`${itemLabel} Düzenlenebilir`,'.docx',index)});return{type:rel,label:itemLabel,index,group:itemMeta.group||null,preview:pdf,webPreview:null,thumbnail:null,editable:docx,downloads}}).filter(x=>x.preview||x.editable||x.downloads.length)}
function optionalWeekFields(source){const result={};for(const key of['icerik_ders_saati','okul_temelli_planlama_saati','dersler','kilavuz','sinif_araclari','ogretmen_notu','ogrenci_aciklama','sinif_akisi','gunluk_plan_yok'])if(Object.hasOwn(source,key))result[key]=source[key];return result}
function normalizeWeek(source,sourceKind){const kazanimlar=Array.isArray(source.kazanimlar)?[...source.kazanimlar]:[],surec=Array.isArray(source.surec_bilesenleri)?[...source.surec_bilesenleri]:[];return{hafta_no:Number(source.hafta_no),baslangic:source.baslangic??'',bitis:source.bitis??'',tarih_araligi:source.tarih_araligi??source.tarih??'',ders_saati:source.ders_saati??null,tema:source.tema??source.unite??'',unite:source.unite??'',konu:source.konu??'',ogrenme_ciktisi:source.ogrenme_ciktisi??kazanimlar.join(' • '),kazanimlar,surec_bilesenleri:surec.length?surec:kazanimlar,etkinlik:source.etkinlik??'',ozel_hafta:Boolean(source.ozel_hafta),kurban_bayrami_cakisiyor:Boolean(source.kurban_bayrami_cakisiyor),belirli_gun:source.belirli_gun??'',...optionalWeekFields(source),sourceKind,source}}
