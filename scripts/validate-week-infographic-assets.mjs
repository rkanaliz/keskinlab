...
for (const [course, week, expected] of checks) {
  const materials = manifest.courses[course].weeks[week].materials.filter((item) => item.type === 'infografik');
  ...
  for (let index = 1; index <= expected; index += 1) {
    ...
    const files = [
      `materyaller/${course}/hafta${week}/infografik/${number}.png`,
      `generated/web/${course}/hafta${week}/infografik/${number}.webp`,
      `generated/web/${course}/hafta${week}/infografik/${number}-thumb.webp`,
    ];
