...
function renderStage(){
  ...
  if(visual)bindVisualFallback(step);bindActivity(step);
}
...
async function load(){
  try{
    ...
    const requested=new URL(location.href).searchParams.get('step'),i=requested?stepIndex(requested):-1;state.index=i>=0?i:0;els.loading.classList.add('hidden');render();
