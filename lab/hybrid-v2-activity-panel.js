...
const icons={
  ...
  email:svg('<rect x="7" y="11" width="34" height="26" rx="2"/><path d="m9 14 15 12 15-12"/>'),
  ...
  atm:svg('<rect x="8" y="7" width="32" height="34" rx="2"/><rect x="13" y="12" width="22" height="10"/><path d="M15 29h18M18 34h5M27 34h4"/>'),
  atm2:svg('<rect x="8" y="7" width="32" height="34" rx="2"/><rect x="13" y="12" width="22" height="10"/><path d="M15 29h18M18 34h5M27 34h4"/>'),
  ...
  phoneedu:svg('<rect x="15" y="5" width="18" height="38" rx="3"/><path d="M20 10h8M22 37h4"/><circle cx="24" cy="23" r="5"/>'),
  watchhealth:svg('<rect x="14" y="12" width="20" height="24" rx="5"/><path d="M19 4h10l2 8H17zM19 44h10l2-8H17zM18 25h4l2-5 3 9 2-4h3"/>'),
  video:svg('<rect x="6" y="10" width="27" height="28" rx="3"/><path d="m33 19 9-5v20l-9-5z"/><circle cx="19" cy="20" r="4"/><path d="M12 32c2-6 12-6 14 0"/>'),
  ...
  camera:svg('<rect x="7" y="13" width="27" height="24" rx="3"/><path d="m34 20 8-5v20l-8-5z"/><circle cx="20" cy="25" r="6"/>'),
  ...
  green:svg('<rect x="7" y="9" width="34" height="28" rx="2"/><path d="m15 30 7-8 5 5 5-7 6 10"/><circle cx="16" cy="17" r="3"/>'),
  ...
  monitor:svg('<rect x="6" y="8" width="36" height="27" rx="2"/><path d="M17 42h14M24 35v7"/>')
};
...
const zoneIcons={
  ...
  goruntu:icons.camera,
  ses:icons.microphone,
  duzenleme:icons.editing,
  efekt:svg('<path d="m24 5 3 10 10 3-10 3-3 10-3-10-10-3 10-3zM37 28l2 6 6 2-6 2-2 6-2-6-6-2 6-2z"/>'),
  yayin:icons.stream,
  egitim:svg('<path d="M24 13c-5-4-11-5-17-3v27c6-2 12-1 17 3 5-4 11-5 17-3V10c-6-2-12-1-17 3z"/><path d="M24 13v27"/>'),
  saglik:icons.hospital,
  ulasim:icons.route,
  finans:icons.atm,
  eglence:icons.game
};
const zoneCode={iletisim:'KV-01',bilgi:'KV-02',bilisim:'KV-03',teknoloji:'KV-04',bit:'KV-05',goruntu:'İŞ-01',ses:'İŞ-02',duzenleme:'İŞ-03',efekt:'İŞ-04',yayin:'İŞ-05',egitim:'AL-01',saglik:'AL-02',ulasim:'AL-03',finans:'AL-04',eglence:'AL-05'};

function enhance(){
  const slide=stage.querySelector('.activity-slide');
  document.body.classList.toggle('activity-focus',!!slide);
  if(!slide||slide.dataset.activityV5==='1')return;
  slide.dataset.activityV5='1';
  slide.classList.add('activity-v5');
  ...
  if(!head||!counter||!pool||!zones||!actions)return;

  const index=document.createElement('div');
  index.className='activity-v5-index';
  index.innerHTML='<div class="index-steps"><span><b>01</b> OKU</span><span><b>02</b> BAĞ KUR</span><span><b>03</b> YERLEŞTİR</span></div><div class="index-rule"><small>ÇALIŞMA KURALI</small><strong>En güçlü ilişkiyi seç.</strong></div>';
  ...
  pool.querySelectorAll('.item-btn').forEach((btn,i)=>{
    ...
    code.textContent=`K-${String(i+1).padStart(2,'0')}`;
    btn.prepend(icon,code);
  });
  ...
  zones.querySelectorAll('.zone').forEach(zone=>{
    ...
    code.textContent=zoneCode[id]||'ALAN';
    const icon=document.createElement('span');
    icon.className='zone-icon';
    icon.innerHTML=zoneIcons[id]||fallbackIcon;
    const label=zone.querySelector('b');
    zone.prepend(code);
    if(label)label.after(icon);else zone.append(icon);
    const drop=document.createElement('span');
    drop.className='zone-drop-label';
    drop.textContent='SEÇİLİ KART → BU ALAN';
    zone.append(drop);
  });

  const note=document.createElement('div');
  note.className='activity-v5-note';
  note.innerHTML='<small>ÇALIŞMA NOTU</small><span>Kartın kısa açıklamasını oku. Kavramın adından çok anlamına göre karar ver.</span>';
